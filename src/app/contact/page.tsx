...
const CONTACT_EMAIL = 'bimmerworks.ro@gmail.com';
...
export default function ContactPage() {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    const subject = encodeURIComponent(`Bimmerworks enquiry — ${form.service || 'General'} — ${form.car}`);
  ...
  return (
    <>
      ...
      >
        <p className="text-accent uppercase tracking-[0.3em] text-sm font-semibold mb-4">Get In Touch</p>
      ...
      <section className="py-8 px-4 bg-section-bg border-b border-white/5">
        <div className="max-w-4xl mx-auto flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-10 text-sm">
          ...
          <a href={`mailto:${CONTACT_EMAIL}`}
            className="flex items-center gap-2 text-gray-300 hover:text-white transition-colors font-medium">
            <span className="text-accent">✉</span> {CONTACT_EMAIL}
          </a>
          <span className="hidden sm:block text-white/20">|</span>
      ...
      <section className="py-20 px-4 bg-background">
        <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div>
            ...
            <p className="text-muted text-sm mb-8">Fill in your details and your email client will open ready to send.</p>

            {submitted ? (
              <div className="bg-card-bg border border-accent/40 rounded-2xl p-10 text-center">
                <div className="text-5xl mb-4">✅</div>
                <h3 className="text-xl font-bold mb-2">Your email client should have opened!</h3>
                ...
                <p className="text-muted text-sm mb-6">For the fastest reply, message us on WhatsApp:</p>
                <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 text-white font-bold px-6 py-3 rounded-lg text-sm transition-all duration-200 hover:scale-105"
                  style={{ background: '#25D366' }}>
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="white">
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                  </svg>
                  Message on WhatsApp — we reply same day
                </a>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-5">
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">Name <span className="text-accent">*</span></label>
                  <input type="text" name="name" required value={form.name} onChange={handleChange}
                    placeholder="Your full name" className={inputClass} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">Email <span className="text-accent">*</span></label>
                  <input type="email" name="email" required value={form.email} onChange={handleChange}
                    placeholder="your@email.com" className={inputClass} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">Phone <span className="text-muted text-xs">(optional)</span></label>
                  <input type="tel" name="phone" value={form.phone} onChange={handleChange}
                    placeholder="+40 7xx xxx xxx" className={inputClass} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">Your Car <span className="text-accent">*</span></label>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">Service <span className="text-accent">*</span></label>
                  <select name="service" required value={form.service} onChange={handleChange}
                    className={inputClass + ' cursor-pointer'}>
                    <option value="" disabled>Select a service</option>
                    <optgroup label="── Fixed Price Packages ──">
                      {services.slice(0, 7).map((s) => <option key={s} value={s} className="bg-card-bg">{s}</option>)}
                    </optgroup>
                    <optgroup label="── Contact for Pricing ──">
                      {services.slice(7).map((s) => <option key={s} value={s} className="bg-card-bg">{s}</option>)}
                    </optgroup>
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">Message <span className="text-accent">*</span></label>
                ...
                <p className="text-center text-muted text-xs">
                  ...
                  <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="text-green-400 hover:underline">WhatsApp</a>
                  {' '}— we reply same day.
                </p>
              </form>
            )}
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-xl p-6"
              ...
              <p className="text-muted text-sm mb-4">Message us directly — we reply same day. Best for quick quotes and compatibility checks.</p>
              <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer"
                ...
                +40 757 226 101 — We reply same day
              </a>
            </div>
            ...
            <div className="bg-card-bg border border-card-border rounded-xl p-6">
              ...
              <p className="text-muted text-sm leading-relaxed">
                You don&apos;t need to be local. Our remote tune process works with any BMW via OBD interface and AnyDesk. We&apos;ll guide you through every step.
              </p>
            </div>
            ...
            <div className="bg-card-bg border border-card-border rounded-xl p-4 flex items-center gap-3">
              ...
              <div>
                ...
                <a href={`mailto:${CONTACT_EMAIL}`} className="text-accent hover:text-white transition-colors text-sm font-medium">
                  {CONTACT_EMAIL}
                </a>
              </div>
            </div>
